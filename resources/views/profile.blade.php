...
@section('content')
	...
	@endif
	<h1 id="#SCR">Student Cumulative Record <small>Guidance and Counselling Office</small></h1>
	<div class="row">			
		<div class="col-10">
			@if($student == NULL)
			...
			@else
				<h3>Personal Data <small><a href="/students/{{$student->id}}/edit">Edit</a></small></h3>
				<table class="table table-hover table-responsive">
					<tbody>
						<tr>
							<td><b>USN</b></td>
		...
		{{-- @if() --}}
			<div>			
				<img src="{{ asset('/img/avatars/'.$user_avatar) }}" alt="..." class="bg ma">
				 {!! Form::open(['enctype' => 'multipart/form-data', 'action' => 'ProfileController@avatar_upload', 'method' => 'POST']) !!}
					<label id="avatar-label" for="avatar" class="avatar-label">
						<i class="fa fa-file-image-o icon-sidebar" aria-hidden="true"></i>Browse Photo
					</label>
					<input id="avatar" type="file" name="avatar">
					{{ csrf_field() }}
					{{-- <i class="fa fa-upload icon-sidebar" aria-hidden="true"></i> --}}

					{{Form::submit('Upload', ['class' => 'btn btn-primary btn-sm', 'style' => 'width: 100%; cursor: pointer;'])}}
				{!! Form::close() !!}
			</div>
		</div>
	</div>
	<script>
		document.getElementById("avatar").addEventListener("change", change_label)
